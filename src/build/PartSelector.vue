...
export default {
  ...
  props: {
    ...
    position: {
      ...
      required: true,
      validator(value) {
        return ['top', 'left', 'center', 'right', 'base'].includes(value);
      },
    },
  },
  data() { return { selectedPartIndex: 0 }; },
  computed: {
    selectedPart() {
  ...
  methods: {
    emitSelectedPart() {
      this.$emit('selectedPartEvent', this.parts[this.selectedPartIndex]);
    },
    selectNextPart() {
